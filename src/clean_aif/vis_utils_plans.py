...
from clean_aif.vis_utils_paths import NUM_POLICIES_VIS
...
def plot_action_seq(
    ...
    plt.savefig(
        save_dir + "/" + f"{env_layout}_actions_plan_run{run_index}.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_reward_counts(file_data_path, x_ticks_estep, save_dir, env_layout):
    ...
    plt.title("Agent solving the task\n")
    ...
    # )
    plt.savefig(
        save_dir + "/" + f"{env_layout}_plan_agents_goal.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_steps_count(file_data_path, x_ticks_estep, save_dir, env_layout):
    ...
    plt.savefig(
        save_dir + "/" + f"{env_layout}_plan_steps_count.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_pi_fe(
    ...
    for p in range(16):
        ...
        avg_pi_fe = np.mean(pi_fe[:, :, p + POLICY_INDEX_OFFSET, :], axis=0).squeeze()
        std_pi_fe = np.std(pi_fe[:, :, p + POLICY_INDEX_OFFSET, :], axis=0).squeeze()
        ...
        plt.figure()
        plt.plot(x1, y1, ".-", label=f"Policy $\\pi_{p + POLICY_INDEX_OFFSET}$")
        ...
        plt.savefig(
            save_dir + "/" + f"pi{p + POLICY_INDEX_OFFSET}_fes.jpg",
            format="jpg",
            bbox_inches="tight",
            pad_inches=0.1,
        )
        ...
        plt.savefig(
            save_dir + "/" + f"pi{p}_fes_plan_last_step.jpg",
            format="jpg",
            bbox_inches="tight",
            pad_inches=0.1,
        )
...
def plot_pi_fe_compare(
    ...
    num_episodes = data["num_episodes"]
    num_policies = data["num_policies"]
    ordered_policies = data["ordered_policies"][0, 0, 0, :, :]
    num_steps = data["num_steps"]
    ...
    fig.savefig(
        save_dir + "/" + f"{env_layout}_policies_fe_plan_every_step.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    ...
    fig.savefig(
        save_dir + "/" + f"{env_layout}_policies_fe_plan_step{step_num}.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_total_fe(
    ...
    - file_data_path (string): file path where the total free energy data was stored (i.e. where log_data
      was saved);
    - step_fe (integer): step for which to visualize the free energy;
    - x_ticks_estep (integer): step for the ticks in the x axis when plotting as a function of episode number;
    - x_ticks_tstep (integer): step for the ticks in the x axis when plotting as a function of total timesteps;
    - save_dir (string): directory where to save the images.
    ...
    plt.figure()
    plt.plot(x1, y1, ".-", label="Free energy")
    plt.xticks(np.arange(0, (num_episodes * num_steps) + 1, step=x_ticks_tstep))
    plt.xlabel("Step")
    plt.ylabel("Free energy", rotation=90)
    # plt.legend(loc="upper right") # No need for legend
    plt.title("Every-step free energy\n")
    # plt.show()
    plt.close()
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_total_fe_plan_step{step_num}.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_pi_prob(file_data_path, x_ticks_tstep, select_policy, save_dir, env_layout):
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_pi_probs_plan_every_step.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_pi_prob_first(
    ...
    plt.savefig(
        save_dir
        + "/"
        + f"{env_layout}_first_pi_probs_plan_offset{POLICY_INDEX_OFFSET}.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_efe(
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_efe_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_efe_comps(
    ...
    # Save figures
    fig_1.savefig(
        save_dir + "/" + f"{env_layout}_efe_plan_risk.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    fig_2.savefig(
        save_dir + "/" + f"{env_layout}_efe_plan_bnov.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    ...
    # Save figure
    fig_3.savefig(
        save_dir + "/" + f"{env_layout}_efe_plan_amb.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    fig_4.savefig(
        save_dir + "/" + f"{env_layout}_efe_plan_anov.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_Qs_pi_prob(
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_Qs_pi_prob_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_Qt_pi_prob(
    ...
    for p in range(num_policies):
        ...
        # Save figure and show
        plt.savefig(
            save_dir + "/" + f"{env_layout}_Qt_pi{p}_prob_plan.jpg",
            format="jpg",
            bbox_inches="tight",
            pad_inches=0.1,
        )
...
def plot_so_mapping(
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_emis_prob_state{s}_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_so_map_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_transitions(
    ...
    plt.savefig(
        save_dir + "/" + f"{env_layout}_tr_probs_state{s}_action{a}_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    ...
    for a in range(0, 4):
        ...
        # Save figure and show
        plt.savefig(
            save_dir + "/" + f"{env_layout}_transmap_plan_a{a}.jpg",
            format="jpg",
            bbox_inches="tight",
            pad_inches=0.1,
        )
...
def plot_Qs_pi_first(file_data_path, select_policy, episode, save_dir, env_layout):
    """Visualising the Q(S_i|pi) for each policy at the first time time step of an episode, where i
    ...
    num_states = data["num_states"]
    num_policies = data["num_policies"]
    # Note: policies are ordered at the first step of every episode, here we retrieve such order by looking
    # at any run/episode; (num_runs, num_episodes, num_steps, num_policies, num_efe_steps)
    ...
    first_step_Qs = np.tile(
        avg_Qs[episode, :, 0][np.newaxis, :, np.newaxis], (num_policies, 1, 1)
    )
    ...
    last_episode_Qspi = np.concatenate((first_step_Qs, last_episode_Qspi), axis=2)
    ...
    for p in range(num_policies):
        ...
        im = ax.pcolormesh(X, Y, last_episode_Qspi[p, :, :], shading="auto")
...
def plot_Qs(file_data_path, episode, save_dir, env_layout):
    """Visualising the Q(S_i) at the last time time step of an episode, where i is in [0,...,num_steps-1]
    and indicates the time step during an episode. Note that the the Q(S_i|pi) are categorical
    distributions telling you the state beliefs the agent has for each episode's time step.
    ...
    # Retrieving the data dictionary and extracting the content of required keys, e.g. 'states_beliefs'
    data = np.load(file_data_path, allow_pickle=True).item()
    num_episodes = data["num_episodes"]
    num_steps = data["num_steps"]
    num_states = data["num_states"]
    ...
    # Heatmap of the Q(s) at the last time step of the episode
    fig, ax = plt.subplots()
    fig.set_figwidth(5)
    fig.set_figheight(6)
    X, Y = np.meshgrid(np.arange(num_steps), np.arange(num_states))
    im = ax.pcolormesh(X, Y, last_episode_Qs[:, :].squeeze(), shading="auto")
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_Qs_ls_ep{episode}_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
def plot_state_visits(file_path, v_len, h_len, select_policy, save_dir, env_layout):
    ...
    # Save figure and show
    plt.savefig(
        save_dir + "/" + f"{env_layout}_sv_plan.jpg",
        format="jpg",
        bbox_inches="tight",
        pad_inches=0.1,
    )
